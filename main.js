...
function login(){app.innerHTML=`<div class="login"><div class="box"><h1>OFICINA <span>MOTUDO SHINERAY</span></h1><p>Login:</p><input id="email" placeholder="E-mail"><input id="pass" type="password" placeholder="Senha"><button id="login">Entrar</button><div id="msg"></div></div></div>`;document.querySelector('#login').onclick=async()=>{if(!sb){msg.textContent='Configure o .env com as credenciais do Supabase.';return}const {error}=await sb.auth.signInWithPassword({email:email.value,password:pass.value});if(error)msg.textContent=error.message;else load()}}
